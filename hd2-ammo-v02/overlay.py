...
def decide_show(fraction: float, fill: int, low: bool, o: dict) -> tuple[float, bool]:
    """余量 + 红色状态 → (要画的填充长度, 是否红)。

    单独抽成纯函数，是为了让运行时（AmmoOverlay.show_value）和离线可视化脚本
    走同一份判定，避免两边口径漂移 —— 之前出过「调参器预览和实跑不一致」的坑。

    low=True 表示识别端已经确认是「红色弹药」（游戏把计量器转红了）。这条路径
    不能省：低弹时白色像素会整条消失，只按占比是判不出来的。
    """
    if fraction <= 0.0 and not low:
        return 0.0, False       # 空弹匣：不画实心条，只剩灰轨
    red = (low
           or fraction < float(o.get("low_threshold", 0.15))
           or (int(o.get("low_white_px", 0)) > 0
               and fill <= int(o.get("low_white_px", 0))))
    full = bool(o.get("low_full_bar", True))
    return (1.0 if (red and full) else fraction), red
...
class AmmoOverlay:
    ...
    def __init__(self, cfg: dict, screen: tuple[int, int]):
        ...
        self._top_at = 0.0
        self.redraws = 0
        self.shown_ever = False      # 本次运行是否上过屏（窗口关掉后仍能如实报告）
        self.hwnd = None
        self._memdc = None
        self._dib = None
    ...
    def status(self) -> str:
        if self.hwnd is None:
            # 收尾时窗口已经关掉了，别报「从未显示过」误导排查
            return ("窗口已关闭（本次运行显示过）" if self.shown_ever
                    else "窗口尚未创建（从未有有效读数）")
        l, t, r, b = self.rect()
    ...
    # -- 便利入口 ------------------------------------------------------------ #
    def show_value(self, fraction: float, fill: int = 0, low: bool = False) -> None:
        """按余量决定画什么：

        - 空弹匣（余量归零、且不是低弹红）→ 不画实心条，只剩灰色轨迹。
        - 低弹 → 整条弧线泛红（不看具体余量多少）。三条触发路径：
            1) `low=True`：识别端直接检出「红色弹药」（游戏把计量器转红了，
               state == "low"）。这是主路径 —— 低弹时白色像素会消失，
               只靠占比是判不出来的。
            2) 余量低于 low_threshold。
            3) 白像素数 <= low_white_px（>0 时生效）。
        - 其余 → 白色填充条，长度 = 余量。

        判定在 decide_show() 里，与本文件顶部的可视化脚本共用。
        """
        disp, red = decide_show(fraction, fill, low, self.cfg)
        self.show(disp, red=red)
