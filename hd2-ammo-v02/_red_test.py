...
# 区域与检测器配对：单一来源 = config 的 regions（v02 起只有一组，无背包靠 layout.shift 平移）
BOXES = [("A", BOX_A)]
if cfg.get("layout", {}).get("enabled"):
    sx, sy = cfg["layout"]["shift"]
    BOXES.append(("A无包", (BOX_A[0] + sx, BOX_A[1] + sy,
                            BOX_A[2] + sx, BOX_A[3] + sy)))
...
def mk(name="T"):
    return ad.RegionDetector(
        ...
        shell_min_iou=D["shell_min_iou"], shell_min_px=D["shell_min_px"])
